...
export function Inbox({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden border-r border-slate-200 bg-white">
      <div className="shrink-0 border-b border-slate-200 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
        ...
        {cases.map((c) => {
          ...
          return (
            ...
            >
              ...
              {c.status === "error" && <span className="text-xs text-rose-600">{c.error}</span>}
            </button>
          );
        })}
      </div>
